// Функция для получения пути сохранения из файла конфигурации
function getLastSavedFolder() {
    var configFile = new File(Folder.myDocuments + "/lastSavedFolder.txt");
    if (configFile.exists) {
        configFile.open("r");
        var savedData = configFile.read();
        configFile.close();
        // Возвращаем путь, если он существует
        return new Folder(savedData);
    }
    return null;
}

// Функция для сохранения пути в файл конфигурации
function saveLastSavedFolder(folder) {
    var configFile = new File(Folder.myDocuments + "/lastSavedFolder.txt");
    configFile.open("w");
    configFile.write(folder.fsName); // Сохраняем только путь как строку
    configFile.close();
}
...
// Загружаем последний сохраненный путь
var lastSavedFolder = getLastSavedFolder();

// Выбираем папку для сохранения PDF файлов
var destFolder = lastSavedFolder ? Folder.selectDialog("Выберите папку для сохранения PDF", lastSavedFolder) : Folder.selectDialog("Выберите папку для сохранения PDF");

if (destFolder) {
    // Сохраняем путь в файл конфигурации для будущего использования
    saveLastSavedFolder(destFolder);
